<!--
ISTRUZIONI (questo blocco è un commento HTML: non verrà mostrato su GitHub)
1. Sostituisci tutti i placeholder tra [parentesi quadre].
2. Aggiorna la tabella "Progetti in evidenza" man mano che completi nuovi lavori.
3. Rimuovi le sezioni che non ti servono ancora e riaggiungile quando avrai contenuti.
4. Badge personalizzabili su https://shields.io
-->

# Ciao, sono Maria Incarnato 👋

🎓 Laureata in **Ingegneria dell'Automazione** – Università degli Studi di Napoli Federico II
🤖 In partenza per la **Laurea Magistrale in Robotica** – Federico II
🔍 Focus su **visione artificiale e percezione robotica**

---

### 🧠 Chi sono

Mi occupo di visione artificiale applicata alla robotica: nella mia tesi triennale, sviluppata presso il PRISMA Lab, ho progettato un sistema di percezione basato su YOLOv8 per un robot umanoide. Sto per iniziare la magistrale in Robotica e cerco di approfondire ulteriormente il campo della percezione robotica, in vista di tesi magistrale, tirocinio o prime esperienze in R&D. <!--[Personalizza con eventuali dettagli in più su di te.]-->

---

### 🛠️ Competenze tecniche

**Linguaggi**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat&logo=mathworks&logoColor=white)

**Robotica & Percezione**

![YOLO](https://img.shields.io/badge/YOLOv8-Ultralytics-6f42c1)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white)
![RealSense](https://img.shields.io/badge/Intel%20RealSense-0071C5?style=flat&logo=intel&logoColor=white)

**Controllo & Automazione**

Teoria dei sistemi · Controllo automatico · PLC · Simulink

**Strumenti**

![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

### 📌 Progetti in evidenza

| Progetto | Descrizione | Tecnologie | Repository |
|---|---|---|---|
| **Tesi triennale — Riconoscimento visivo per un robot umanoide nei giochi da tavolo** | Sistema di object detection (YOLOv8) + stima di distanza (RealSense) per un Unitree H1-2, testato su un vero gioco da tavolo presso il PRISMA Lab | Python · YOLOv8 · OpenCV · RealSense | [tesi-riconoscimento-visivo-robot-umanoide](#) |
<!-- | 🔜 [Prossimo progetto di visione artificiale] | In lavorazione durante la magistrale | — | — |  -->

---

### 📫 Contatti

<!-- [LinkedIn](https://linkedin.com/in/...) -->· [Email](maryincarnato18@gmail.com) · <!-- [CV (PDF)](link) -->
